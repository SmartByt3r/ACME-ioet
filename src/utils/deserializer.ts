...
const employeeWorkDayRegEx = new RegExp(`${employeeNameRegEx.source}=${workDayRegEx.source}(,${workDayRegEx.source})*$`);
...
export function deserializeEmployees(employeesData: string):Employee[] {
    ...
    if(entries.length === 0) throw new Error("No data found");

    return entries.map((entry,rowNumber) => {
        if(!employeeWorkDayRegEx.test(entry)) throw new Error(`Invalid data format at entry number ${rowNumber+1}`);
        
        const [employeeName, workedScheduleData] = entry.split('=');
        if(!employeeName || !workedScheduleData) throw new Error("Invalid data format!");
